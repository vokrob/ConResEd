...
export default function App() {
    ...
    async function toggleFavorite(id) {
        const item = savedResumes.find((resume) => resume.id === id);
        if (!item) return;
        const nextFavorite = !item.is_favorite;

        setSavedResumes((prev) =>
            prev.map((resume) => (resume.id === id ? { ...resume, is_favorite: nextFavorite } : resume))
        );

        try {
            const res = await fetch(`${API_BASE_URL}/resumes/${id}/favorite`, {
                method: "PATCH",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(data.error || "Не удалось обновить избранное");
            const persistedFavorite = !!data?.item?.is_favorite;
            setSavedResumes((prev) =>
                prev.map((resume) => (resume.id === id ? { ...resume, is_favorite: persistedFavorite } : resume))
            );
        } catch (err) {
            setSavedResumes((prev) =>
                prev.map((resume) => (resume.id === id ? { ...resume, is_favorite: item.is_favorite } : resume))
            );
            setStatus({ message: err.message || "Ошибка сети", type: "error" });
        }
    }
        ...
        <div className="container">
            ...
            <div className="dashboard-grid">
                        ...
                        {filteredAndSortedResumes.length === 0 ? (
                            ...
                            filteredAndSortedResumes.map((item) => (
                                <div key={item.id} className="template-row" role="listitem">
                                    ...
                                    <button type="button" className="template-open-btn saved-template-open-btn" onClick={() => navigate(`/templates/${item.template_id}?resumeId=${item.id}`)}>
                                        <div className="template-row-content">
                                    ...
                                    <div className="resume-actions">
                                        <button type="button" className="btn btn-primary btn-compact" onClick={() => navigate(`/templates/${item.template_id}?resumeId=${item.id}`)}>Открыть</button>
                ...
                <section className="demo-section template-section">
                    ...
                    <div className="template-list" role="list">
                        {templates.map((t) => (
                            <div key={t.id} className="template-row template-choice-row" data-variant={t.variant} role="listitem">
                                <button type="button" className="template-open-btn template-choice-open-btn" onClick={() => navigate(`/templates/${t.id}?blank=1`)}>
                                    <div className="template-row-content">
                                        <strong>{t.title}</strong>
                                        <span>{t.description}</span>
                                    </div>
                                </button>
                                <button type="button" className="btn btn-primary btn-compact template-choice-action-btn" onClick={() => navigate(`/templates/${t.id}?blank=1`)}>
                                    Открыть
                                </button>
                            </div>
                        ))}

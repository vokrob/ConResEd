...
import {
    MODERN_PREFIX, modernDescriptors, modernSkillsBase,
    remapModernRemoveEducationRow, remapModernRemoveExperienceRow,
    remapModernRemoveLang, remapModernRemoveSkill,
...
export default function ModernTemplate() {
    ...
    const descriptors = useMemo(() => modernDescriptors(experienceCount, educationCount, skillCount, langCount), [experienceCount, educationCount, skillCount, langCount]);
    ...
    const removeExperienceAt = (rowIndex) => {
        ...
        const next = remapModernRemoveExperienceRow(fieldValues, rowIndex, experienceCount, educationCount, skillCount, langCount);
    ...
    const removeEducationAt = (rowIndex) => {
        ...
        const next = remapModernRemoveEducationRow(fieldValues, rowIndex, experienceCount, educationCount, skillCount, langCount);
        replaceFieldValues(next);
    ...
    const removeSkillAt = (rowIndex) => {
        ...
        const next = remapModernRemoveSkillAt(fieldValues, experienceCount, educationCount, skillCount, langCount, rowIndex);
        replaceFieldValues(next);
        setSkillCount((n) => n - 1);
    };
    ...
    const removeLanguageAt = (rowIndex) => {
        ...
        const next = remapModernRemoveLangAt(fieldValues, experienceCount, educationCount, skillCount, langCount, rowIndex);
        replaceFieldValues(next);
        setLangCount((n) => n - 1);
    };

    const clearAll = useCallback(() => { clearCtrl(); setSkillCount(4); setLangCount(1); }, [clearCtrl]);
    const structure = { experience: experienceCount, education: educationCount };

    const handleSave = async (title) => await saveToCabinet("modern", keys, structure, title, false);
    const handleSaveCopy = async (title) => await saveToCabinet("modern", keys, structure, title, true);
    ...
    return (
        <>
            <TemplateNav
                ...
                currentTitle="Мое резюме (Современный)"
                ...
                <div className="resume-container">
                    ...
                    <div className="content one-column">
                            ...
                            <div className="contact-grid">
                                {["Email", "Телефон", "Город", "hh.ru"].map((label, i) => (
                                    <div key={label} className="contact-item">
                                        ...
                                        <EditableField fieldKey={contactKeys[i]} value={fieldValues[contactKeys[i]]} onChange={setField} placeholder={label} />
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="section print-priority-high">
                            ...
                            <div className="summary-box"><EditableField fieldKey={aboutKey} value={fieldValues[aboutKey]} onChange={setField} placeholder="Расскажите о себе" /></div>
                        </div>
                            ...
                            <div id="skills-container" className="skills-container">
                                {skillKeys.map((sk, idx) => (
                                    <span key={sk} className="skill-tag" style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: "8px" }}>
                                        ...
                                        {!readOnly && <button type="button" className="item-remove-btn" onClick={() => removeSkillAt(idx)} disabled={skillCount <= 1} style={{ margin: 0, width: "24px", height: "24px", fontSize: "14px" }}>−</button>}
                                    </span>
                                ))}
                            </div>
                                ...
                                {Array.from({ length: langCount }).map((_, row) => {
                                    ...
                                    return (
                                        <div key={`lang-${row}`} className="language-item" style={{ display: "flex", alignItems: "center", gap: "12px", justifyContent: "space-between" }}>
                                            ...
                                            {!readOnly && <button type="button" className="item-remove-btn" onClick={() => removeLanguageAt(row)} disabled={langCount <= 1} style={{ margin: 0 }}>−</button>}
                                            <div className="progress-bar"><div className="progress-fill" style={{ width: "100%" }} /></div>
                                        </div>
                                    );
                                })}
                            </div>
                            {!readOnly && <div className="experience-controls"><button type="button" className="btn-add" onClick={addLanguage}>+</button></div>}
                        </div>

                        <div className="section">
                            <h2 className="section-title">Дополнительно</h2>
                            <EditableField fieldKey={interestsKey} value={fieldValues[interestsKey]} onChange={setField} placeholder="Ваши интересы" />
                        </div>
                    </div>
                </div>
            </div>

...
/** Имя, должность, слоган, 3 пустых метрики */
const CREATIVE_HEADER = 8;
/** Контактов в левой колонке */
const CREATIVE_CONTACTS = 5;

/** Индекс первого поля блока «Опыт» (после шапки, контактов, навыков, пустого блока и «О себе»). */
export function creativeExpBase(skillCount) {
  return CREATIVE_HEADER + CREATIVE_CONTACTS + skillCount + 2;
}

/** Порядок полей как в legacy creative.html (depth-first). */

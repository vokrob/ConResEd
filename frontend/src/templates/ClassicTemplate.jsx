...
export default function ClassicTemplate() {
    ...
    const handleSave = async (title) => {
        await saveToCabinet("classic", keys, structure, title, false);
    };

    const handleSaveCopy = async (title) => {
        await saveToCabinet("classic", keys, structure, title, true);
    };
    ...
    return (
        <>
            <TemplateNav
                ...
                currentTitle="Мое резюме (Классический)"
                onResumeSave={handleSave}
                onResumeCopy={handleSaveCopy}
            />

            <div className="classic-template-page">
                        ...
                        <div className="header-meta-below print-priority-high" aria-label="Должность и контакты">
                            ...
                            <div className="header-contacts" aria-label="Контакты">
                                <div className="header-contact-row">
                                    <EditableField fieldKey={headerKeys[4]} value={fieldValues[headerKeys[4]]} onChange={setField} placeholder="Email" />
                                </div>
                                <div className="header-contact-row">
                                    <EditableField fieldKey={headerKeys[5]} value={fieldValues[headerKeys[5]]} onChange={setField} placeholder="Телефон" />
                                </div>
                                <div className="header-contact-row">
                                    <EditableField fieldKey={headerKeys[6]} value={fieldValues[headerKeys[6]]} onChange={setField} placeholder="Город" />
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div key={`exp-${row}`} className="experience-item">
                                        <div className="item-header">
                                            ...
                                            <span className="item-date">
                                                <EditableField fieldKey={rk[1]} value={fieldValues[rk[1]]} onChange={setField} placeholder="Период работы" multiline={false} maxLength={48} />
                                            </span>
                                        </div>
                                        <div className="item-company">
                                            <EditableField fieldKey={rk[2]} value={fieldValues[rk[2]]} onChange={setField} placeholder="Название компании" />
                                        </div>
                                        <div className="item-description">
                                            <EditableField fieldKey={rk[3]} value={fieldValues[rk[3]]} onChange={setField} placeholder="Описание обязанностей" />
                                        </div>
                                        {!readOnly && (
                                            <button type="button" className="item-remove-btn" onClick={() => removeExperienceAt(row)}>−</button>
                                        )}
                                    </div>
                                );
                            })}
                            ...
                            {Array.from({ length: educationCount }).map((_, row) => {
                                ...
                                return (
                                    <div key={`edu-${row}`} className="education-item">
                                        <div className="item-header">
                                            <span className="item-title">
                                                <EditableField fieldKey={rk[0]} value={fieldValues[rk[0]]} onChange={setField} placeholder="Степень / Специальность" />
                                            </span>
                                            <span className="item-date">
                                                <EditableField fieldKey={rk[1]} value={fieldValues[rk[1]]} onChange={setField} placeholder="Год окончания" multiline={false} maxLength={24} />
                                            </span>
                                        </div>
                                        <div className="item-company">
                                        ...
                                        {!readOnly && (
                                            <button type="button" className="item-remove-btn" onClick={() => removeEducationAt(row)}>−</button>
                                        )}
                                    </div>
                                );
                            })}
                        ...
                        <div id="skills-container" className="skills-list">
                            {skillKeys.map((sk, row) => (
                                <div key={sk} className="skill-row">
                                    ...
                                    {!readOnly && (
                                        <button type="button" className="item-remove-btn" onClick={() => removeSkillAt(row)} disabled={skillCount <= 1}>−</button>
                                    )}
                                </div>
                            ))}
                        </div>

...
export default function ClassicTemplate() {
    ...
    const removeExperienceAt = (rowIndex) => {
        ...
        const next = remapAfterRemoveExperienceRow(fieldValues, rowIndex, experienceCount, educationCount, skillCount);
    ...
    const removeEducationAt = (rowIndex) => {
        ...
        const next = remapAfterRemoveEducationRow(fieldValues, rowIndex, experienceCount, educationCount, skillCount);
        replaceFieldValues(next);
    ...
    const handleSaveConfirm = async (title) => {
        ...
        const result = await saveToCabinet("classic", keys, structure, title, isCopy);
        if (result?.ok) {
            setSaveStatus({ message: result.message, visible: true, type: "success" });
        } else {
            setSaveStatus({ message: result.message || "Ошибка сохранения", visible: true, type: "error" });
        }
        ...
        <div className="classic-template-page" lang="ru">
            ...
            <div className="resume-container">
                <header className="header print-priority-high">
                    {!readOnly && <PhotoUploader onPhotoSelect={setPhoto} currentPhoto={photo} />}
                    {readOnly && photo && <div className="avatar-placeholder"><img src={photo} alt="Фото" /></div>}
                    {readOnly && !photo && <div className="avatar-placeholder">📷 Фото</div>}
                ...
                <section className="section print-priority-high">
                    ...
                    <div className="summary">
                        <EditableField fieldKey={headerKeys[7]} value={fieldValues[headerKeys[7]]} onChange={setField} placeholder="Краткое описание вашего профессионального опыта" />
                    </div>
                </section>
                            ...
                            return (
                                <div key={`exp-${row}`} className="experience-item">
                                    <div className="item-header">
                                        ...
                                        <span className="item-date"><EditableField fieldKey={rk[1]} value={fieldValues[rk[1]]} onChange={setField} placeholder="Период работы" multiline={false} maxLength={48} /></span>
                                    </div>
                                    <div className="item-company"><EditableField fieldKey={rk[2]} value={fieldValues[rk[2]]} onChange={setField} placeholder="Название компании" /></div>
                                    <div className="item-description"><EditableField fieldKey={rk[3]} value={fieldValues[rk[3]]} onChange={setField} placeholder="Описание обязанностей" /></div>
                            ...
                            return (
                                <div key={`edu-${row}`} className="education-item">
                                    <div className="item-header">
                                        ...
                                        <span className="item-date"><EditableField fieldKey={rk[1]} value={fieldValues[rk[1]]} onChange={setField} placeholder="Год окончания" multiline={false} maxLength={24} /></span>
                                    </div>
                    ...
                    <div id="skills-container" className="skills-list">
                        {skillKeys.map((sk, row) => (
                            <div key={sk} className="skill-row">
                                ...
                                {!readOnly && <button type="button" className="item-remove-btn" onClick={() => removeSkillAt(row)} disabled={skillCount <= 1}>−</button>}
                            </div>
                        ))}
                    </div>
                ...
                <section className="section print-priority-low">
                    ...
                    <EditableField fieldKey={extraKey} value={fieldValues[extraKey]} onChange={setField} placeholder="Языки, сертификаты, курсы" />
                </section>
            </div>

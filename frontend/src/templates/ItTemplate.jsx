...
import {
    itDescriptors, itExpBase, itEduBase, itTechBase,
    itProjectBase, itCertBase,
    remapItRemoveExperienceRow, remapItRemoveEducationRow,
    remapItRemoveTechAt, remapItRemoveProjectAt, remapItRemoveCertAt,
...
export default function ItTemplate() {
    ...
    const descriptors = useMemo(() => itDescriptors(experienceCount, educationCount, techCount, projectCount, certCount), [experienceCount, educationCount, techCount, projectCount, certCount]);
    ...
    const removeExperienceAt = (rowIndex) => {
        ...
        const next = remapItRemoveExperienceRow(fieldValues, rowIndex, experienceCount, educationCount, techCount, projectCount, certCount);
    ...
    const removeEducationAt = (rowIndex) => {
        ...
        const next = remapItRemoveEducationRow(fieldValues, rowIndex, experienceCount, educationCount, techCount, projectCount, certCount);
        replaceFieldValues(next);
    ...
    const removeTechAt = (rowIndex) => {
        ...
        const next = remapItRemoveTechAt(fieldValues, experienceCount, educationCount, techCount, projectCount, certCount, rowIndex);
        replaceFieldValues(next);
        setTechCount((n) => n - 1);
    };
    ...
    const removeProjectAt = (rowIndex) => {
        ...
        const next = remapItRemoveProjectAt(fieldValues, experienceCount, educationCount, techCount, projectCount, certCount, rowIndex);
        replaceFieldValues(next);
    ...
    const removeCertAt = (rowIndex) => {
        ...
        const next = remapItRemoveCertAt(fieldValues, experienceCount, educationCount, techCount, projectCount, certCount, rowIndex);
        replaceFieldValues(next);
        setCertCount((n) => n - 1);
    };

    const clearAll = useCallback(() => { clearCtrl(); setTechCount(5); setProjectCount(1); setCertCount(1); }, [clearCtrl]);
    const structure = { experience: experienceCount, education: educationCount };

    const handleSave = async (title) => await saveToCabinet("it", keys, structure, title, false);
    const handleSaveCopy = async (title) => await saveToCabinet("it", keys, structure, title, true);

    if (!ready) return <div style={{ textAlign: "center", padding: 40 }}>Загрузка…</div>;

    return (
        <>
            <TemplateNav
                currentResumeId={resumeId}
                currentTitle="Мое резюме (IT Специалист)"
            ...
            <div className="it-template-page">
                <div className="resume-container">
                    <div className="header">
                        ...
                        <div className="header-meta-below">
                        ...
                        <div className="header-contacts">
                            <div className="header-contact-row">✉ <EditableField fieldKey={headerKeys[4]} value={fieldValues[headerKeys[4]]} onChange={setField} placeholder="Email" /></div>
                            <div className="header-contact-row">📱 <EditableField fieldKey={headerKeys[5]} value={fieldValues[headerKeys[5]]} onChange={setField} placeholder="Телефон" /></div>
                            <div className="header-contact-row">📍 <EditableField fieldKey={headerKeys[6]} value={fieldValues[headerKeys[6]]} onChange={setField} placeholder="Город" /></div>
                            <div className="header-contact-row">🔗 <EditableField fieldKey={headerKeys[7]} value={fieldValues[headerKeys[7]]} onChange={setField} placeholder="hh.ru" /></div>
                        </div>
                    </div>

                    <section className="section"><h2 className="section-title">О себе</h2><div className="summary-box"><EditableField fieldKey={aboutKey} value={fieldValues[aboutKey]} onChange={setField} placeholder="Краткое описание вашего опыта в IT" /></div></section>
                    ...
                    <section className="section"><h2 className="section-title">Дополнительно</h2><EditableField fieldKey={extraKey} value={fieldValues[extraKey]} onChange={setField} placeholder="Языки, дополнительные курсы" /></section>
                </div>
            </div>
        </>
    );

...
export function useResumeTemplateController() {
    ...
    const saveToCabinet = useCallback(
        async (templateId, keys, structure, title, isCopy = false) => {
            ...
            const isUpdate = !isCopy && resumeId;
            const url = isUpdate ? `/api/resumes/${resumeId}` : "/api/resumes";
            const method = isUpdate ? "PUT" : "POST";
            const bodyData = {
                template_id: templateId,
                title,
                payload: { fields, structure },
            };
            try {
